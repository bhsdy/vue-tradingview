        ...
        methods: {
          ...
          websocketonmessage (evt) { // 数据接收
            ...
            if(evt.data instanceof Blob){
              ...
              reader.onload = function() {
                ...
                if(_this.result.ping){
                  //响应服务器，避免断开连接
                  _this.webSockt.send(JSON.stringify({pong:_this.result.ping}));
                }
              }
              //将返回的数据解析为字符串格式
            ...
            if(_this.result.data != null){
              // this.bars.push(_this.result.data)
              this.bars.push({
                close: Number(_this.result.data.close),
                open: Number(_this.result.data.open),
                high: Number(_this.result.data.high),
                low: Number(_this.result.data.low),
                volume: Number(_this.result.data.volume24),
                time: Number(_this.result.data.time),
              })
            }
            this.changePair();
            console.log(_this.result.data)
          },
          websocketclose (e) { // 关闭
